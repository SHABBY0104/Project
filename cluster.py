...
#
plt.barh(y=np.arange(1,1+len(col)*3,3), width=mean_c0, label = 'cluster0', color = 'b', alpha = 1, height= bar_width)
plt.barh(y=np.arange(1,1+len(col)*3,3)+bar_width*1, width=mean_c1, label = 'cluster1', color = 'r', alpha = 1, height= bar_width)
plt.barh(y=np.arange(1,1+len(col)*3,3)+bar_width*2, width=mean_c2, label = 'cluster2', color = 'g', alpha = 1., height= bar_width)
plt.barh(y=np.arange(1,1+len(col)*3,3)+bar_width*3, width=mean_c3, label = 'cluster3', color = 'orange', alpha = 1, height= bar_width)
plt.barh(y=np.arange(1,1+len(col)*3,3)+bar_width*4, width=mean_c4, label = 'cluster4', color = 'm', alpha = 1, height= bar_width)

# plt.barh(y=np.arange(len(col))+bar_width, width=cluster2,label='C语言基础', color='indianred', alpha=0.8, height=bar_width)


for y, x in enumerate(mean_c0):
...
for y, x in enumerate(mean_c3):
    plt.text(x+5000, y-bar_width/2, '%s' % x, ha='center', va='bottom')

for y, x in enumerate(mean_c4):
    plt.text(x+5000, y-bar_width/2, '%s' % x, ha='center', va='bottom')

plt.yticks(np.arange(1,1+len(col)*3,3)+bar_width*5/2, col)

plt.axvline(x = 0.1, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
plt.axvline(x = 0.2, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
plt.axvline(x = 0.3, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
plt.axvline(x = 0.4, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
plt.axvline(x = 0.5, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
plt.axvline(x = 0.6, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
plt.axvline(x = 0.7, c = 'b', ls = '--', lw = 0.5,alpha = 0.8)
